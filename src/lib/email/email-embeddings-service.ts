...
import { isSentFolder } from '@/lib/email/imap-mailboxes';
...
function getReplyTextForEmbedding(email: ProcessedEmail): string | null {
  const draft = email.draft_reply?.trim();
  if (draft) return draft;

  if (email.draft_status === 'sent' || isSentFolder(email.folder)) {
    const body = getEmailBodyForAi(email.body_text, email.body_html).trim();
...
function buildEmbeddingInput(email: ProcessedEmail): string {
  const reply = getReplyTextForEmbedding(email) ?? '';
...
export async function embedSentEmailReply(
  ...
  const replyText = getReplyTextForEmbedding(email);
  if (!replyText) return;
...
export async function backfillSentReplyEmbeddings(limit = 40): Promise<number> {
  ...
  const emails = await EmailService.listRecentSent(limit);
