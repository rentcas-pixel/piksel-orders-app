...
export async function chatWithEmailAgent(input: {
  ...
  const styleGuide = await getWritingStylePrompt();
  ...
  const systemParts = [
    ...
    'Turi pilną konteksto paketą: dabartinis laiškas, gija, kliento istorija, panašūs ankstesni atsakymai.',
  ...
  if (styleGuide) {
    systemParts.push(`Papildomas stilius:\n${styleGuide}`);
  }
