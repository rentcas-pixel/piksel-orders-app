...
export async function POST(request: Request) {
  ...
  if (!apiKey) {
    return NextResponse.json({ error: 'MISTRAL_API_KEY nerastas serveryje.' }, { status: 500 });
  }
  ...
  try {
    ...
    const data = await processReceivedInvoiceOcr(apiKey, buffer, file.name, mimeType);
