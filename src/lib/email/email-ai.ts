...
export async function generateEmailReplyWithContext(
  ...
  const systemParts = [
    ...
    languageInstruction,
    formattingRules,
    buildFewShotSystemInstruction(context.replyLanguage, context.fewShotExamples.length),
    context.writingStyleRules,
    'Principai yra antriniai — pirmiausia imituok few-shot pavyzdžius.',
  ...
  const fewShotBlock = formatFewShotBlockForPrompt(
