...
import {
  examplesFromSimilarReplies,
  examplesFromStoredCorpus,
  examplesFromSentEmails,
...
import { getStoredFewShotCorpus } from '@/lib/email/email-style-service';
...
function assessContextStrength(input: {
  ...
  if (input.similarCount >= 2) score += 1;
  if (input.fewShotCount >= 3) score += 2;
  else if (input.fewShotCount === 0) {
    gaps.push('Few-shot pavyzdžių nerasta — paleisk stiliaus mokymą iš Sent/Archive.');
  } else {
    gaps.push('Mažai few-shot pavyzdžių stiliui.');
  }
...
export async function buildEmailContextPackage(
  ...
  const recipientAddress = email.from_address?.trim() ?? '';
  const [sentToRecipient, sentToDomain, storedCorpus] = await Promise.all([
    recipientAddress ? EmailService.listSentToRecipient(recipientAddress, 4) : Promise.resolve([]),
    domain ? EmailService.listSentToDomain(domain, 4) : Promise.resolve([]),
    getStoredFewShotCorpus(),
  ]);

  const fewShotExamples = selectFewShotExamples([
    ...examplesFromThreadMessages(threadMessages),
    ...examplesFromSentEmails(sentToRecipient, 'recipient', 10),
    ...examplesFromSentEmails(sentToDomain, 'domain', 20),
    ...examplesFromSimilarReplies(similarReplies),
    ...examplesFromStoredCorpus(storedCorpus),
  ]);
  ...
  const similarBlock =
    context.similarReplies.length > 0
      ? context.similarReplies
          .map(
            (item, index) =>
              `### Panašumas ${index + 1} (${(item.similarity * 100).toFixed(0)}%)
Konteksto tema: ${item.context_subject ?? '—'}
Atsakymo tema: ${item.subject ?? '—'}
${truncateText(item.reply_text, 400)}`
          )
          .join('\n\n')
...
## 4. PANAŠIOS ANKSTESNĖS TEMOS (papildomas kontekstas)
${similarBlock}

## 5. KONTEKSTO KOKYBĖ
Stiprumas: ${context.contextStrength}
Few-shot pavyzdžių: ${context.fewShotExamples.length}
